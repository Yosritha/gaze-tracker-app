...
# ─────────────────────────────────────────────
# BATTERY HTML — full 10-task JS engine (FIXED)
# ─────────────────────────────────────────────
...
/* Stimulus overlay — FIXED: always has stim-text and stim-label children */
#stim {
    position: absolute;
    inset: 0;
    display: none;
...
<div id="stage">
    ...
    <div id="dot"></div>
    <div id="cross">+</div>
    <!-- FIXED: stim-box always contains stim-text and stim-label.
         Visual search must never overwrite stim.innerHTML — it uses its own canvas instead. -->
    ...
    <!-- FIXED: separate overlay for visual search hint — never touches stim -->
    <div id="vs-hint" style="position:absolute; bottom:0; left:0; right:0; z-index:16; display:none;
         padding:14px; text-align:center; background:rgba(30,45,53,0.85);">
...
let keyHandler = null;   // single global key listener slot — always cleaned by hideAll()
...
// FIXED hideAll: resets ALL overlays to clean state.
// Critically — does NOT touch stim.innerHTML, only hides/resets visibility.
function hideAll() {
    // Remove stale global key listener
    if (keyHandler) {
        document.removeEventListener('keydown', keyHandler);
        keyHandler = null;
    }

    // Hide gaze primitives
    document.getElementById('dot').style.display = 'none';
    document.getElementById('cross').style.display = 'none';

    // FIXED: hide stim without destroying its children
    const stim = document.getElementById('stim');
    stim.style.display = 'none';
    stim.style.alignItems = 'center';
    stim.style.justifyContent = 'center';
    // Reset stim-text and stim-label content but keep the elements
    const stimText = document.getElementById('stim-text');
    const stimLabel = document.getElementById('stim-label');
    if (stimText) { stimText.innerText = ''; stimText.style.color = ''; stimText.style.fontSize = '48px'; }
    if (stimLabel) { stimLabel.innerText = ''; }
    // Reset stim-box to standard styling
    const stimBox = document.getElementById('stim-box');
    if (stimBox) {
        stimBox.style.background = '#1E3040';
        stimBox.style.border = '1.5px solid #D4A843';
        stimBox.style.padding = '30px 50px';
        stimBox.style.minWidth = '300px';
        stimBox.style.borderRadius = '10px';
    }
    ...
    // Remove any dynamically appended canvases from visual search
    const stage = document.getElementById('stage');
    stage.querySelectorAll('canvas:not(#trail-canvas)').forEach(c => c.remove());
}
...
async function initBattery() {
    try {
        ...
        showUI(
            ...
            'Grant webcam permission and reload. The gaze tasks require camera access.',
            'Retry',
            initBattery
        );
    }
        ...
        () => {
            ...
            function runTrial() {
                ...
                setTimeout(() => {
                    ...
                    i++;
                    // Brief 250ms flash per Munoz & Everling 2004
                    setTimeout(() => {
                        dot.style.display = 'none';
...
// FIXED: uses its own appended canvas; never touches stim.innerHTML
// ══════════════════════════════════════════
function task_visualsearch() {
    ...
    showUI(
        ...
        () => {
            ...
            // FIXED: own canvas appended dynamically — never overwrites stim
            ...
            // FIXED: separate hint bar — never overwrites stim
            ...
            function noOverlap(positions, x, y) {
                return positions.every(p => Math.hypot(p[0] - x, p[1] - y) > 55);
            }

            function drawSearch(hasTarget) {
                ctx.clearRect(0, 0, 800, 560);
                const positions = [];
                // Draw 11 blue square distractors
                for (let d = 0; d < 11; d++) {
                    let x, y, tries = 0;
                    do {
                        ...
                        tries++;
                    } while (!noOverlap(positions, x, y) && tries < 50);
                    positions.push([x, y]);
                ...
                // Optionally draw orange circle target
                if (hasTarget) {
                    let x, y, tries = 0;
                    do {
                        x = 60 + Math.random() * 680;
            ...
            function nextTrial() {
                if (trialIdx >= trials.length) {
                    ...
                    // FIXED: remove keyHandler before leaving
                    if (keyHandler) {
                        document.removeEventListener('keydown', keyHandler);
                        keyHandler = null;
                    }
            ...
            keyHandler = handleKey;  // FIXED: store so hideAll() can clean up
    ...
    showUI(
        ...
        () => {
            ...
            function nextTrial() {
                ...
                // Jittered 1.2–3.2s foreperiod prevents anticipation
...
//
// Bugs fixed:
//   1. stim.innerHTML was destroyed by visual search; now rebuilt here.
//   2. stim alignItems was 'flex-end' after visual search; reset in hideAll().
//   3. stim.style.display set inside runTrial (per trial) not at function top.
//   4. keyHandler properly tracked inside resp so hideAll works.
// ══════════════════════════════════════════
function task_gonogo() {
    ...
    showUI(
        ...
        () => {
            ...
            // ── FIX 1: Rebuild stim DOM that visual search destroyed ──────
            // We do this instead of relying on innerHTML being intact,
            // because task_visualsearch() in the original code wiped stim.innerHTML.
            // After this patch task_visualsearch no longer touches stim.innerHTML,
            // but we rebuild defensively here anyway.
            const stim = document.getElementById('stim');
            ...
            // ── FIX 2: Reset stim layout to clean centred state ───────────
            stim.style.alignItems    = 'center';
            stim.style.justifyContent = 'center';
            stim.style.display       = 'none';   // start hidden; shown per-trial below

            const stimText  = document.getElementById('stim-text');
            const stimLabel = document.getElementById('stim-label');

            // 75% Go, 25% No-Go — ref: Aron (2007)
            const sequence = [];
            for (let k = 0; k < 30; k++) {
                sequence.push(Math.random() < 0.75 ? 'GO' : 'NOGO');
            }
            ...
            function runTrial() {
                if (idx >= sequence.length) {
                    // ── FIX 3: Hide stim when task ends ──────────────────
                    stim.style.display = 'none';
                    log('TASK_END', 'GoNoGo');
                    task_nback();
                    return;
                ...
                // 800ms response window — ref: Donders paradigm
                const timeout = setTimeout(() => {
                    if (!responded && type === 'GO') {
                        log('GNG_OMISSION', 'trial:' + (idx - 1));
                    }
                    stim.style.display = 'none';
                    stimText.innerText = '';
                ...
                // ── FIX 6: Scoped resp function — no collision with keyHandler ─
                function resp(e) {
                    if (e.code !== 'Space') return;
                    responded = true;
                ...
                keyHandler = resp;  // allow hideAll() to clean up if needed
            }
...
function task_nback() {
    ...
    function runNBack(n, onDone) {
        ...
        for (let k = 0; k < 20 + n; k++) {
            if (k >= n && Math.random() < 0.3) {
                seq.push(seq[k - n]); // 30% targets
            } else {
        ...
        function showItem() {
            ...
            // 1600ms display, 400ms ISI — ref: Jaeggi 2008
            const timeout = setTimeout(() => {
    ...
    showUI(
        ...
        () => {
            ...
            runNBack(1, () => {
                showUI(
                    'Task 6 · Part B',
                    'Now 2-back: press SPACE when the letter matches the one shown TWO steps ago.',
                    'Begin 2-Back',
                    () => {
                        hideUI();
                        runNBack(2, () => {
...
function task_stroop() {
    ...
    showUI(
        ...
        () => {
            ...
            for (let k = 0; k < 16; k++) {
                const c = COLORS[k % 4];
                trials.push({ word: c, ink: c, congruent: true });
            }
            for (let k = 0; k < 16; k++) {
...
function task_trail() {
    ...
    function runTrail(part, onDone) {
        ...
        function noOverlap(x, y) {
            return nodes.every(p => Math.hypot(p.x - x, p.y - y) > 70);
        }
        ...
        for (let k = 0; k < N; k++) {
            ...
            if (part === 'A') {
                ...
                label = (k % 2 === 0)
                    ? String(k / 2 + 1)
                    : String.fromCharCode(65 + Math.floor(k / 2));
            }
            nodes.push({ x, y, label, visited: false });
        }

        // Correct order for B: 1,A,2,B,...
        let correctOrder;
        if (part === 'A') {
        ...
        function draw() {
            ...
            nodes.forEach(n => {
                ...
                ctx.strokeStyle = n.visited ? '#4CAF82' : '#4A6070';
                ctx.lineWidth   = 2;
                ctx.fill();
                ctx.stroke();
            ...
            ctx.textAlign = 'left';
            ctx.fillText(
                'Trail Making Part ' + part + ' — connect in order: ' + (part === 'A' ? '1→2→3...' : '1→A→2→B→3...'),
                16, 24
            );
        }
    ...
    showUI(
        ...
        () => {
            ...
            runTrail('A', () => {
                showUI(
                    'Trail Making Part B',
                    'Now alternate: 1→A→2→B→3→C... Click as fast as possible.',
                    'Begin Part B',
                    () => {
                        hideUI();
                        runTrail('B', () => {
...
function task_corsi() {
    ...
    showUI(
        ...
        () => {
            ...
            lightUp([0, 1], runTrial); // practice sequence
        }
    );
}
        ...
        () => {
            ...
            function runTrial() {
                ...
                function showDigit() {
                    ...
                    disp.innerText = seq[i];
                    i++;
                    // 800ms per digit — ref: Wechsler 1997
...
def run_analysis(video_file, logs: list) -> dict:
    ...
    D_CM            = 60.0    # viewing distance cm
    PX_PER_CM       = 1920 / 34.5  # ~55.6 px/cm
    ...
    # Merge adjacent fixations within 50ms gap; minimum duration 100ms
    MIN_FIX_DUR = 100.0
    fixations   = []
    buf         = []
    ...
    for ev in logs:
        if ev['event'] == 'ANTISAC_STIM':
            ...
            side = 'RIGHT'
            if 'side:' in ev['details']:
                side = ev['details'].split('side:')[1].split(',')[0]
            next_sac = next((s for s in saccades if s['t_start'] >= ev_t), None)
            if next_sac:
                lat = next_sac['t_start'] - ev_t
                # Valid latency window ref: Munoz & Everling 2004
                if 80 < lat < 1000:
    ...
    # ── 6. GO/NO-GO FEATURES (F11, F12) ──────
    # Reference: Aron (2007), Trends Cogn Sci
    gng_go_trials   = sum(1 for e in logs if e['event'] == 'GNG_STIM' and 'type:GO'   in e['details'] and 'NOGO' not in e['details'])
    gng_nogo_trials = sum(1 for e in logs if e['event'] == 'GNG_STIM' and 'type:NOGO' in e['details'])
    ...
    f13_dprime, f14_bias = compute_dprime(all_nback)

    # ── 8. STROOP FEATURES (F15–F18) ─────────
    # Reference: MacLeod (1991), Psych Bull 109(2): 163-203
    stroop_cong_rt, stroop_incong_rt = [], []
    stroop_errors, stroop_total      = 0, 0
    ...
    for ev in logs:
        if ev['event'] == 'STROOP_RESPONSE':
            ...
            try:
                ...
                cong    = 'cong:True'     in d
                if 200 < rt < 3000:
                    (stroop_cong_rt if cong else stroop_incong_rt).append(rt)
                if not correct:
                    stroop_errors += 1
                stroop_total += 1
            except Exception:
    ...
    # ── 9. TRAIL MAKING FEATURES (F19–F21) ───
    # Reference: Reitan (1958); Lezak (2004) Neuropsychological Assessment
    def get_trail_time(part):
        end_ev = next((e for e in logs if e['event'] == 'TRAIL_END' and 'part:' + part in e['details']), None)
        if end_ev:
            try:
                return float(end_ev['details'].split('time_s:')[1])
            except Exception:
                pass
        return 0.0
    ...
    # ── 10. CORSI & DIGIT SPAN (F22, F23) ────
    def get_max_span(end_event_name):
        ev = next((e for e in logs if e['event'] == end_event_name), None)
        if ev:
            try:
                return int(ev['details'].split('max_span:')[1])
            except Exception:
                pass
        return 0

    f22_corsi    = get_max_span('CORSI_END')
    f23_digitspan = get_max_span('DIGITSPAN_END')

    # ── 11. VISUAL SEARCH FEATURES (F24, F25) ─
    # Reference: Treisman & Gelade (1980), Cognitive Psychology
    ...
    for ev in logs:
        if ev['event'] == 'VS_RESPONSE':
            ...
            try:
                ...
                if 100 < rt < 5000:
                    vs_rts.append(rt)
                if not correct and resp == 'ABSENT':
                    vs_misses += 1
            except Exception:
                pass
...
def classify(key, val):
    ...
    inverted_keys = {
        "F7_AntiSaccadeErrorRate", "F11_CommissionErrors", "F12_OmissionErrors",
        "F18_StroopErrorRate",     "F25_VisualSearchMissRate",
        "F9_RT_Mean",  "F10_RT_IIV",
...
def generate_pdf(participant: dict, results: dict) -> bytes:
    ...
    pdf.set_text_color(160, 179, 188)
    for header, width in [("Feature", 45), ("Domain", 35), ("Value", 30), ("Normal Range", 40), ("Status", 30)]:
    ...
    pdf.multi_cell(0, 5,
        "DISCLAIMER: This report is generated for research purposes only and does not constitute a clinical diagnosis. "
